...
impl ObjectiveMaster {
    ...
    pub fn relaxed_ik(num_chains: usize) -> Self {
        ...
        objectives.push(Box::new(MinimizeVelocity));   weight_priors.push(2.0); // 5.0
        objectives.push(Box::new(MinimizeAcceleration));    weight_priors.push(1.0); // 4.0
        objectives.push(Box::new(MinimizeJerk));    weight_priors.push(0.1); // 3.0
        objectives.push(Box::new(JointLimits));    weight_priors.push(0.1); // 2.0
        objectives.push(Box::new(NNSelfCollision));    weight_priors.push(2.0); // 5.0
