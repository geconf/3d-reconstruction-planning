...
impl RelaxedIK {
    ...
    pub fn from_yaml_path(fp: String, mode: usize) -> Result<Self, std::io::Error> {
        ...
        let result = std::panic::catch_unwind(
            std::panic::AssertUnwindSafe(|| {
                ...
                RelaxedIKVars::from_yaml_path(fp.clone(), false, false)
            })
        );
    ...
    pub fn solve_precise(&mut self, ee_sub: &EEPoseGoalsSubscriber, max_iter: u32) -> Vec<f64> {
        let mut out_x = self.vars.xopt.clone();
        ...
        for i in 0..max_iter {
            ...
            if max_pos_error < 0.001 && max_rot_error < 0.001 {
            // if max_pos_error < 0.001{
                break;
            }

            // solve with self.solve
            self.groove.optimize(&mut out_x, &self.vars, &self.om, 100);
            self.vars.update(out_x.clone());
        }
    ...
    pub fn solve_precise_old(&mut self, ee_sub: &EEPoseGoalsSubscriber) -> (Vec<f64>) {
        let mut out_x = self.vars.xopt.clone();
